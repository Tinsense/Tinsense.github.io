...
function render(){
  ...
  data.forEach((d,i)=>{
    const cx=X(d.cl),cy=Y(d.s),z=zone(d),c=z===1?'#117a65':z===2?'#2859d9':z===3?'#7c3aed':'#c56b18';let mark;
    ...
    const show=()=>{selected=d.m;render();tip.innerHTML=`<b>${d.m}</b><br>M–Cl: ${fmt(d.cl)}<br>M–S: ${fmt(d.s)}<br>Δ: ${d.delta>=0?'+':''}${fmt(d.delta)}<br>S<sub>Li</sub>: ${d.liScore>=0?'+':''}${fmt(d.liScore)}<br>S<sub>Na</sub>: ${d.naScore>=0?'+':''}${fmt(d.naScore)}<br>${ztxt(d)}`;tip.style.display='block'};
